...
beforeAll(async () => {
  ...
  licenseKeyId = data?.data.find((item) => item.attributes.key === LicenseKey)!.id!
})

describe('Activate a license key', () => {
  it('Should return a response', async () => {
    const { statusCode, error: _error, data: _data } = await activateLicense(LicenseKey, 'Test')
    ...
    // license_key
    const { id, status, key, activation_limit, activation_usage, created_at, expires_at, test_mode } = license_key
    const items = [id, status, key, activation_limit, activation_usage, created_at, expires_at, test_mode]
...
describe('Failed to activate a license key', () => {
  ...
  it('Should return a response with 404 statusCode and a error when it fails', async () => {
    const { statusCode, error: _error, data: _data } = await activateLicense(`${LicenseKey}B`, 'Test')
  ...
  // it('Should return a response with 422 statusCode and a error when it fails', async () => {
  //   const {
  //     statusCode,
  //     error: _error
  //     data: _data,
...
describe('Validate a license key', () => {
  it('Should return a response that does not contain an instance', async () => {
    const { statusCode, error: _error, data: _data } = await validateLicense(LicenseKey)
    expect(statusCode).toEqual(200)
    expect(_error).toBeNull()
    expect(_data).toBeDefined()
  ...
  it('Should return a response containing the instance', async () => {
    const { statusCode, error: _error, data: _data } = await validateLicense(LicenseKey, instanceId)
    expect(statusCode).toEqual(200)
    expect(_error).toBeNull()
    expect(_data).toBeDefined()
...
describe('Failed to validate a license key', async () => {
  ...
  it('Should return a response with 404 statusCode and a error when it fails', async () => {
    const { statusCode, error: _error, data: _data } = await validateLicense(`${LicenseKey}B`)
  ...
  // it('Should return a response with 422 statusCode and a error when it fails', async () => {
  //   const { statusCode, error: _error, data: _data } = await validateLicense('')
...
describe('Failed to deactivate a license key', async () => {
  ...
  it('Should return a response with 400 statusCode and a error when it fails', async () => {
    ...
    const { statusCode, error: _error, data: _data } = await deactivateLicense(LicenseKey, instanceId)
  ...
  it('Should return a response with 404 statusCode and a error when it fails', async () => {
    const { statusCode, error: _error, data: _data } = await deactivateLicense(`${LicenseKey}B`, instanceId)
