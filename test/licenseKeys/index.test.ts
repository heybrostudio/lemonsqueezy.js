...
import { getLicenseKey, lemonSqueezySetup, listLicenseKeys, updateLicenseKey } from '../../src'
...
describe('List all license keys', () => {
  it('Should return all license keys', async () => {
    ...
    const { store_id, order_id, order_item_id, product_id, status } = attributes
    licenseKeyId = id
    storeId = store_id
    orderId = order_id
  ...
  it('Should return all license keys with related resources', async () => {
    const { statusCode, error, data: _data } = await listLicenseKeys({ include: ['order'] })
    expect(statusCode).toEqual(200)
    expect(error).toBeNull()
    expect(_data).toBeDefined()
  ...
  it('Should return all license keys filtered by status', async () => {
    const { statusCode, error, data: _data } = await listLicenseKeys({ filter: { status: licenseKeyStatus } })
    expect(statusCode).toEqual(200)
    expect(error).toBeNull()
    expect(_data).toBeDefined()
    ...
    const { meta, data, links } = _data!
    expect(meta.page).toBeDefined()
    expect(data.filter((item) => item.attributes.status === licenseKeyStatus).length).toEqual(data.length)
    expect(links.first).toBeDefined()
    expect(links.last).toBeDefined()
...
describe('Retrieve a license key', () => {
  ...
  it('Should return a license key object', async () => {
    const { statusCode, error, data: _data } = await getLicenseKey(licenseKeyId)
    expect(statusCode).toEqual(200)
    expect(error).toBeNull()
    expect(_data).toBeDefined()
    ...
    const relationshipItems = [store, customer, order, orderItem, product, licenseKeyInstances]
  ...
  it('Should return a license key object with related resources', async () => {
    const { statusCode, error, data: _data } = await getLicenseKey(licenseKeyId, { include: ['order'] })
    expect(statusCode).toEqual(200)
    expect(error).toBeNull()
    expect(_data).toBeDefined()
