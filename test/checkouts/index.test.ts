...
import { createCheckout, getCheckout, lemonSqueezySetup, listCheckouts, listVariants } from '../../src'
...
describe('Create a checkout', () => {
  ...
  it('A new checkout should be created with the given store id and variant id', async () => {
    const { error, data: _data, statusCode } = await createCheckout(storeId, variantId)
    ...
    expect(Object.keys(product_options).length).toEqual(productOptionItems.length)
    ...
    expect(Object.keys(checkout_options).length).toEqual(checkoutOptionItems.length)
  ...
  it('A new checkout should be created with the given store id, variant id and new checkout info', async () => {
    ...
    const { error, data: _data, statusCode } = await createCheckout(storeId, variantId, newCheckout as NewCheckout)
    expect(error).toBeNull()
    expect(statusCode).toEqual(201)
    expect(_data).toBeDefined()
    ...
    expect(receipt_button_text).toEqual(newCheckout.productOptions.receiptButtonText)
    expect(receipt_link_url).toEqual(newCheckout.productOptions.receiptLinkUrl)
    expect(receipt_thank_you_note).toEqual(newCheckout.productOptions.receiptThankYouNote)
    ...
    expect(subscription_preview).toEqual(newCheckout.checkoutOptions.subscriptionPreview)
...
describe('Retrieve a checkout', () => {
  ...
  it('Should return the checkout with the given checkout id and related resources', async () => {
    const { error, data: _data, statusCode } = await getCheckout(newCheckoutId, { include: ['store'] })
    expect(error).toBeNull()
    expect(statusCode).toEqual(200)
    expect(_data).toBeDefined()
...
describe('List all checkouts', () => {
  ...
  it('Should return a paginated list of checkouts with related resources', async () => {
    const { error, data: _data, statusCode } = await listCheckouts({ include: ['store'] })
    expect(error).toBeNull()
    expect(statusCode).toEqual(200)
    expect(_data).toBeDefined()
